...
public class UpgradeService {
    ...
    public Response setTargetVersion(@QueryParam("version") String version, @QueryParam("force") String forceGeoUpgrade) throws IOException {
        ...
        try {
            ...
            String BACKUP_TAG_TEMPLATE = "PreUpgrade-%s-%s";
            String backupTag = String.format(BACKUP_TAG_TEMPLATE, current.toString(), backupTime);
            backupOps.createBackup(backupTag, true);
        } catch (Exception e) {
            _log.error("Failed to create backup before upgrade", e);
